...
:FORM:BORD NORM
:TRAC? TRACE%s"""    
# 'FORM INT, 32'   this command specifies the data format for the data transmitted from the instrument to the control PC. It is used for the transmission of trace data.    20120417 LjHe

class SLF(GPIBManagedServer):
    name = 'Spectrum Analyzer Server'
    deviceName = 'Rohde&Schwarz FSL-18'
    ...
    def peak_frequency(self, c, data):
        ...
        dev = self.selectedDevice(c)
        pos = float((yield dev.query(':CALC:MARK:X?')))   # check the x position of the marker   20120417  LjHe
