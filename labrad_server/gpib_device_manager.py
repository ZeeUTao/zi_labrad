...
def parseIDNResponse(s):
    ...
    s_utf8 = str(eval(s),'utf-8')
    mfr, model, ver, rev = s_utf8.split(',')
    res = mfr.strip() + ' ' + model.strip()
    return res
...
class GPIBDeviceManager(LabradServer):
    ...
    def refreshDeviceLists(self):
        ...
        servers = [s for n, s in list(self.client.servers.items())
    ...
    def gpib_device_connect(self, gpibBusServer, channel):
        ...
        print('idnResult----',device,idnResult)
        if device == UNKNOWN:
    ...
    def lookupDeviceName(self, server, channel):
        ...
        try:
            ...
            resp_utf8 = str(eval(resp),'utf-8')
        except Exception as e:
            print('Error sending *IDN? to', server, channel + ':', e)
            name = UNKNOWN
        returnValue((name, resp_utf8))
    ...
    def register_server(self, c, devices, messageID):
        ...
        for device in devices:
            ...
            for (server, channel), (known_device, idnResult) in list(self.knownDevices.items()):
