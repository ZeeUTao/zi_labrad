...
class GPIBDeviceManager(LabradServer):
    ...
    def gpib_device_connect(self, server, channel):
        ...
        print 'Device servers: ',str(self.deviceServers)
        print 'Known device servers are:', self.deviceServers
    ...
    def lookupDeviceName(self, server, channel):
        ...
        #p.address(channel).timeout(1000).write('*CLS').write('*IDN?').read()
        p.address(channel).timeout(1000).write('*IDN?').read()
        # p.address(channel).timeout(1000).query('*IDN?')
    ...
    def identifyDevicesWithServer(self, identifier):
        ...
        def _doServerIdentify():
            ...
            print '111111111111111', self.knownDevices
    ...
    def register_server(self, c, devices, messageID):
        ...
        for device in devices:
            ...
            for (server, channel), (known_device, idnResult) in self.knownDevices.items():
                print '%%%%', server, channel, known_device, idnResult, '%%%%'
    ...
    @setting(12)
    def dump_device_servers(self, c):
        return str(self.deviceServers)
