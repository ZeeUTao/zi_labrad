...
class zurich_hd(LabradServer):
    ...
    @setting(3, 'awg_builder', waveform=['*2v'],ports=['*v'],awg_index=['v'])
    def awg_builder(self,c,waveform=[[0]],ports=[],awg_index=0):
        """
    ...
    @setting(4, 'awg_upload_string', awg_program=['s'],awg_index=['v'])
    def awg_upload_string(self,c,awg_program, awg_index = 0): 
        """"写入波形并编译
